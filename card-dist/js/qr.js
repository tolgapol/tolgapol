export function renderQr(text, errorLevel = 'L') {
  ...
  try {
    ...
    target.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${n} ${n}" role="img" aria-label="vCard QR code" shape-rendering="crispEdges"><rect width="${n}" height="${n}" fill="#fff"/><path d="${d}" fill="currentColor"/></svg>`;
  } catch {
    target.textContent = '';
  }
}
